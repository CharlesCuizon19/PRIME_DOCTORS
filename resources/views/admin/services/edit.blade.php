...
@section('admin-content')
    <div class="flex justify-center items-center min-h-[85vh] bg-gray-50">
        <div class="w-full max-w-screen-2xl bg-white p-8 rounded-2xl shadow-lg border border-gray-100">
            <h1 class="text-3xl font-extrabold text-gray-800 mb-8 border-b pb-3">Edit Service</h1>
            ...
            <form action="{{ route('admin.services.update', $service->id) }}" method="POST" enctype="multipart/form-data"
                ...
                {{-- Why It Matters --}}
                <div>
                    <label for="why_it_matters" class="block text-lg font-semibold text-gray-700 mb-2">Why It
                        Matters</label>
                ...
                {{-- ✅ Benefits --}}
                <div>
                    <label class="block text-lg font-semibold text-gray-700 mb-2">Select Benefits</label>
                    <div id="benefits-container" class="space-y-3">
                        @php
                            $selectedBenefits = old('benefits', $service->benefits->pluck('id')->toArray() ?? []);
                        @endphp

                        @forelse($selectedBenefits as $index => $benefitId)
                                ...
                                <select name="benefits[]" class="flex-1 border border-gray-300 rounded-lg px-4 py-2">
                                    ...
                                    @foreach ($benefits as $benefit)
                                        <option value="{{ $benefit->id }}"
                                            {{ $benefit->id == $benefitId ? 'selected' : '' }}>
                                            {{ $benefit->benefit_name }}
                                        </option>
                                    @endforeach
                                </select>

                                {{-- If it's the first row, show +, else − --}}
                                @if ($loop->first)
                                    <button type="button"
                                        class="add-benefit px-3 py-2 bg-[#FBD55B] text-black font-semibold rounded-lg hover:bg-[#e4c14e] transition">
                                        +
                                    </button>
                                @else
                                    <button type="button"
                                        class="remove-benefit px-3 py-2 bg-red-500 text-black font-semibold rounded-lg hover:bg-red-600 transition">
                                        −
                                    </button>
                                @endif
                            </div>
                ...
                {{-- ✅ Inclusions --}}
                <div>
                    <label class="block text-lg font-semibold text-gray-700 mb-2">Select Inclusions</label>
                    <div id="inclusions-container" class="space-y-3">
                        @php
                            $selectedInclusions = old('inclusions', $service->inclusions->pluck('id')->toArray() ?? []);
                        @endphp

                        @forelse($selectedInclusions as $index => $inclusionId)
                            <div class="flex gap-2">
                                <select name="inclusions[]" class="flex-1 border border-gray-300 rounded-lg px-4 py-2">
                                    ...
                                    @foreach ($inclusions as $inclusion)
                                        <option value="{{ $inclusion->id }}"
                                            {{ $inclusion->id == $inclusionId ? 'selected' : '' }}>
                                ...
                                @if ($loop->first)
                                    <button type="button"
                                        class="add-inclusion px-3 py-2 bg-[#FBD55B] text-black font-semibold rounded-lg hover:bg-[#e4c14e] transition">
                                        +
                                    </button>
                                @else
                                    <button type="button"
                                        class="remove-inclusion px-3 py-2 bg-red-500 text-white font-semibold rounded-lg hover:bg-red-600 transition">
                                        −
                                    </button>
                                @endif
                            </div>
                ...
                {{-- Service Image --}}
                <div
                    class="border-2 border-dashed rounded-xl p-8 text-center bg-gray-50 hover:bg-gray-100 transition relative">
                    <p class="font-semibold text-gray-700 mb-2">Service Image</p>
                    ...
                    <div id="service_image_preview_container" class="hidden mt-4">
                        ...
                        <div id="service_image_preview" class="relative inline-block rounded-lg overflow-hidden shadow-md">
                        </div>
                    </div>
                </div>
                ...
                <div
                    class="border-2 border-dashed rounded-xl p-8 text-center bg-gray-50 hover:bg-gray-100 transition relative">
                    <p class="font-semibold text-gray-700 mb-2">Icon Image</p>

                    @if ($service->icon && $service->icon->file && $service->icon->file->image_path)
                        <img src="{{ asset('storage/' . $service->icon->file->image_path) }}"
                            class="w-24 h-24 mx-auto mb-4 object-cover rounded-lg shadow-lg">
                    @endif
                    ...
                    <div id="icon_image_preview_container" class="hidden mt-4">
                        <p class="font-semibold mb-2">Preview:</p>
                        <div id="icon_image_preview" class="relative inline-block rounded-lg overflow-hidden shadow-md">
                        </div>
                    </div>
                </div>
            ...
            document.addEventListener('DOMContentLoaded', () => {
                ...
                benefitsContainer.addEventListener('click', (e) => {
                    if (e.target.matches('.add-benefit')) {
                        ...
                        btn.textContent = '−';
                        btn.classList.replace('add-benefit', 'remove-benefit');
                    ...
                    if (e.target.matches('.remove-benefit')) {
                        if (benefitsContainer.children.length > 1) e.target.closest('div.flex').remove();
                    }
                });

                // 🔁 Inclusions dynamic add/remove
                ...
                inclusionsContainer.addEventListener('click', (e) => {
                    if (e.target.matches('.add-inclusion')) {
                        ...
                        const btn = newRow.querySelector('button');
                        btn.textContent = '−';
                        btn.classList.replace('add-inclusion', 'remove-inclusion');
                        btn.classList.replace('bg-[#FBD55B]', 'bg-red-500');
                        btn.classList.add('hover:bg-red-600');
                        inclusionsContainer.appendChild(newRow);
                    }
                    if (e.target.matches('.remove-inclusion')) {
                        if (inclusionsContainer.children.length > 1) e.target.closest('div.flex').remove();
                    }
                });

                // 🖼 Preview Setup (same as create)
