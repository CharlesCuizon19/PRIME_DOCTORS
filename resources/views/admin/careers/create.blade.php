...
@section('title', 'Careers / Create Career')
    ...
    <div class="flex justify-center items-center min-h-[85vh] bg-gray-50">
        <div class="w-full max-w-screen-2xl bg-white p-8 rounded-2xl shadow-lg border border-gray-100">
            ...
            <form action="{{ route('admin.careers.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                {{-- Career Name --}}
                <div>
                    <label for="career_name" class="block text-lg font-semibold text-gray-700 mb-2">
                    ...
                    <input type="text" name="career_name" id="career_name"
                        ...
                        value="{{ old('career_name') }}" placeholder="Enter career name" required>
                    @error('career_name')
                ...
                {{-- Job Type --}}
                <div>
                    <label for="job_type" class="block text-lg font-semibold text-gray-700 mb-2">
                        Job Type <span class="text-red-500">*</span>
                    </label>
                    <input type="text" name="job_type" id="job_type"
                        class="block w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-[#FBD55B] focus:border-[#FBD55B]"
                        value="{{ old('job_type') }}" placeholder="Enter job type" required>
                    @error('job_type')
                        <p class="text-red-600 text-lg mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                {{-- Experience --}}
                <div>
                    <label for="experience" class="block text-lg font-semibold text-gray-700 mb-2">Experience</label>
                ...
                {{-- Vacancy --}}
                <div>
                    <label for="vacancy" class="block text-lg font-semibold text-gray-700 mb-2">Vacancy</label>
                    <input type="number" name="vacancy" id="vacancy"
                        class="block w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-[#FBD55B] focus:border-[#FBD55B]"
                        value="{{ old('vacancy') }}" placeholder="Number of vacancies">
                    @error('vacancy')
                        <p class="text-red-600 text-lg mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                {{-- Overview --}}
                <div>
                    <label for="overview" class="block text-lg font-semibold text-gray-700 mb-2">Overview</label>
                    <textarea name="overview" id="overview" rows="5"
                        class="block w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-[#FBD55B] focus:border-[#FBD55B]"
                        placeholder="Describe this career">{{ old('overview') }}</textarea>
                    @error('overview')
                        <p class="text-red-600 text-lg mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                {{-- Qualifications --}}
                <div>
                    <label class="block text-lg font-semibold text-gray-700 mb-2">Select Qualifications</label>
                ...
                {{-- Responsibilities --}}
                <div>
                    <label class="block text-lg font-semibold text-gray-700 mb-2">Select Responsibilities</label>
                        ...
                        <div class="flex gap-2">
                            <select name="responsibility_ids[]" class="flex-1 border border-gray-300 rounded-lg px-4 py-2">
                                ...
                                @foreach ($responsibilities as $responsibility)
                                    <option value="{{ $responsibility->id }}">{{ $responsibility->responsibility }}
                                    </option>
                                @endforeach
                            </select>
                            <button type="button"
                ...
                <div
                    ...
                    <p class="text-lg text-gray-500 mb-4">Accepted formats: JPG, PNG, WEBP — Max size: 2MB</p>
                    ...
                    @error('career_image')
                        <p class="text-red-600 text-lg mt-2">{{ $message }}</p>
                    @enderror
                </div>
