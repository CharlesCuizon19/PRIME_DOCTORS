    ...
    <div class="flex justify-center items-center min-h-[85vh] bg-gray-50">
        <div class="w-full max-w-screen-2xl bg-white p-8 rounded-2xl shadow-lg border border-gray-100">
            ...
            <form action="{{ route('admin.doctors.update', $doctor->id) }}" method="POST" enctype="multipart/form-data"
                ...
                {{-- Gender --}}
                <div>
                    <label for="gender" class="block text-lg font-semibold text-gray-700 mb-2">
                ...
                {{-- Clinic Room --}}
                <div>
                    <label for="clinic_room_number" class="block text-lg font-semibold text-gray-700 mb-2">
                        Clinic Room Number
                    </label>
                ...
                {{-- Clinic Hours --}}
                <div>
                    <label for="clinic_hours" class="block text-lg font-semibold text-gray-700 mb-2">Clinic Hours</label>
                    <select name="clinic_hours" id="clinic_hours"
                        ...
                        @foreach (['Mon–Fri 8AM–12PM', 'Mon–Fri 1PM–5PM', 'Mon–Fri 9AM–6PM', 'Mon–Sat 9AM–6PM', 'Sat 9AM–1PM', 'Sun 10AM–3PM', 'Daily 8AM–8PM'] as $hour)
                            <option value="{{ $hour }}"
                                {{ old('clinic_hours', $doctor->clinic_hours) == $hour ? 'selected' : '' }}>
                                {{ $hour }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                {{-- Phone & Telephone --}}
                <div>
                    <label for="phone_num" class="block text-lg font-semibold text-gray-700 mb-2">Mobile Number</label>
                    <input type="text" name="phone_num" id="phone_num"
                        class="block w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-blue-400 focus:border-blue-400"
                        value="{{ old('phone_num', $doctor->phone_num) }}">
                </div>
                <div>
                    <label for="telephone_num" class="block text-lg font-semibold text-gray-700 mb-2">Telephone
                        Number</label>
                ...
                {{-- Specializations --}}
                <div>
                    <label class="block text-lg font-semibold text-gray-700 mb-2">Specializations <span
                            class="text-red-500">*</span></label>
                    <div id="specializations-container" class="space-y-3"
                        data-spec-count="{{ $doctor->specializations->count() }}">
                        @foreach ($doctor->specializations as $index => $spec)
                            <div class="flex gap-2 specialization-row">
                                <select name="specializations[{{ $index }}][id]"
                                    class="flex-1 border border-gray-300 rounded-lg px-4 py-2">
                                    <option value="">-- Choose Specialization --</option>
                                    @foreach ($specializations as $s)
                                        <option value="{{ $s->id }}" {{ $spec->id == $s->id ? 'selected' : '' }}>
                                            {{ $s->specialization_name }}</option>
                                    @endforeach
                                </select>
                                <select name="specializations[{{ $index }}][type]"
                                    class="flex-1 border border-gray-300 rounded-lg px-4 py-2">
                                    <option value="Primary" {{ $spec->pivot->type == 'Primary' ? 'selected' : '' }}>Primary
                                    </option>
                                    <option value="Secondary" {{ $spec->pivot->type == 'Secondary' ? 'selected' : '' }}>
                                        Secondary</option>
                                </select>
                                <button type="button"
                                    class="remove-specialization px-3 py-2 bg-red-500 text-white font-semibold rounded-lg hover:bg-red-600 transition">−</button>
                            </div>
                        @endforeach
                    </div>
                    <button type="button" id="add-specialization"
                        class="px-3 py-2 bg-blue-500 text-white font-semibold rounded-lg hover:bg-blue-600 transition mt-2">+
                        Add Specialization</button>
                </div>

                {{-- Languages --}}
                <div>
                    <label class="block text-lg font-semibold text-gray-700 mb-2">Languages</label>
                            ...
                            <div class="flex gap-2 language-row">
                                <select name="languages[]" class="flex-1 border border-gray-300 rounded-lg px-4 py-2">
                                    ...
                                    @foreach ($languages as $l)
                                        <option value="{{ $l->id }}" {{ $lang->id == $l->id ? 'selected' : '' }}>
                                            {{ $l->language }}</option>
                                    @endforeach
                                </select>
                                <button type="button"
                                    class="remove-language px-3 py-2 bg-red-500 text-white font-semibold rounded-lg hover:bg-red-600 transition">−</button>
                            </div>
                        @endforeach
                    </div>
                ...
                <div
                    ...
                    <p class="text-lg text-gray-500 mb-4">Accepted formats: JPG, PNG, WEBP (max 2MB)</p>
                    ...
                    @if ($doctor->image && $doctor->image->file && $doctor->image->file->image_path)
                        <div class="mt-4">
                            <p class="text-lg text-gray-600 mb-1">Current Image:</p>
                            <img src="{{ asset($doctor->image->file->image_path) }}" alt="{{ $doctor->name }}"
                ...
                <div class="flex justify-between items-center pt-4">
                    ...
                    <button type="submit"
                        class="px-6 py-2 bg-blue-500 text-white font-semibold rounded-lg shadow hover:bg-blue-600 transition">Update
                        Doctor</button>
                </div>
            </form>
        </div>
        ...
        `;
            ...
            imageInput.addEventListener('change', e => {
                ...
                reader.onload = e => {
                    ...
                    previewContainer.innerHTML =
                        `<img src="${e.target.result}" class="w-40 h-40 object-cover rounded-lg shadow-lg">`;
                };
