        ...
        <table class="table-fixed w-full border-collapse text-center" id="users-table">
            <thead>
                <tr class="bg-black text-white text-lg font-semibold">
                    ...
                    <th class="px-10 py-3 text-center w-1/4">Subject</th>
                    <th class="px-10 py-3 text-center w-1/4">Message</th>
            ...
            <tbody>
                @forelse($contacts as $contact)
                    <tr class="border-t">
                        ...
                        <td class="px-10 py-3">{{ strip_tags($contact->phone_num) }}</td>
                        <td class="px-10 py-3 uppercase">{{ strip_tags($contact->subject) }}</td>
                        <td class="px-10 py-3">{{ strip_tags($contact->message) }}</td>
                        <td class="px-6 py-3 whitespace-nowrap">
                            <div class="flex justify-center items-center gap-2">
                                <button type="button" onclick="confirmDelete({{ $contact->id }})"
                                    class="px-3 py-1 rounded text-white" style="background-color:#EF4444;">
                                    Delete
                                </button>
                            </div>
                        </td>
