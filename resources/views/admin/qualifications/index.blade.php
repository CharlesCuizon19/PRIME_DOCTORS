...
<!-- Page Title -->
<h1 class="text-2xl font-semibold mb-6">Qualifications</h1>
...
<div class="overflow-x-auto bg-white border rounded-lg p-4">
    <table id="qualifications-table" class="min-w-full border-collapse">
        <thead>
            <tr class="bg-black text-white text-sm font-semibold">
                <th class="px-10 py-3 text-center w-1/4">ID</th>
                <th class="px-10 py-3 text-center w-1/2">Qualification Name</th>
        ...
        <tbody>
            ...
            <tr class="border-t">
                <td class="px-10 py-3 text-center">{{ $qualification->id }}</td>
                <td class="px-10 py-3 text-center">{{ $qualification->qualification }}</td>
                <td class="px-6 py-3 text-center">
                    <form action="{{ route('admin.qualifications.destroy', $qualification->id) }}" method="POST" class="inline-block delete-form">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
...
<div id="create-modal" class="fixed inset-0 bg-black bg-opacity-50 hidden flex items-center justify-center z-50">
    <div class="bg-white rounded-3xl shadow-lg p-8 w-[600px] max-w-full">
        <h2 class="text-lg font-semibold mb-6">CREATE QUALIFICATION</h2>

        <form method="POST" action="{{ route('admin.qualifications.store') }}">
            @csrf
            <div class="mb-4">
                <label for="qualification" class="block text-sm font-medium text-gray-700">Qualification Name</label>
...
<script>
    $(document).ready(function() {
        ...
        $('.delete-form').on('submit', function(e) {
            ...
            Swal.fire({
                title: 'Are you sure?',
                text: "This qualification will be permanently deleted!",
                icon: 'warning',
    ...
    function closeCreateModal() {
        ...
        document.getElementById('qualification_name').value = '';
    }
</script>
@endpush